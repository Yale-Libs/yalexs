...
def test_alarm_attributes() -> None:
...
def test_alarm_missing_required_field_raises() -> None:
    data = _alarm_data()
...
def test_alarm_device_attributes() -> None:
...
def test_alarm_device_status_flags_truthy() -> None:
    dev = AlarmDevice(
        _alarm_device_data(
            status_overrides={
                "online": False,
...
def test_alarm_device_status_flags_missing_keys_default_false() -> None:
    data = _alarm_device_data()
    data["status"] = {"firmwareVersion": "1.2.3"}
...
def test_alarm_device_battery_level_normal() -> None:
    dev = AlarmDevice(_alarm_device_data())
    assert dev.battery_level == 100


def test_alarm_device_battery_level_low() -> None:
    dev = AlarmDevice(_alarm_device_data(status_overrides={"lowBattery": True}))
    assert dev.battery_level == 10


def test_alarm_device_battery_level_missing_low_battery_key() -> None:
    data = _alarm_device_data()
...
def test_alarm_device_repr() -> None:
    dev = AlarmDevice(_alarm_device_data())
    rep = repr(dev)
    assert "dev-1" in rep
...
def test_alarm_device_raw_data_preserved() -> None:
    data = _alarm_device_data()
    dev = AlarmDevice(data)
    assert dev.raw is data


def test_alarm_device_missing_pubsub_channel_is_none() -> None:
    data = _alarm_device_data()
...
def test_alarm_device_missing_required_field_raises() -> None:
    data = _alarm_device_data()
    del data["alarmID"]
    with pytest.raises(KeyError):
        AlarmDevice(data)
